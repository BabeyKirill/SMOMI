...
def augmented_train(image, label):
    image = tf.image.random_crop(image, size=[140, 160, 3], seed=None, name=None)
    image = tf.image.resize_images(image, tf.constant([RESIZE_TO, RESIZE_TO]))
    return image,label
...
def main():
    ...
    model.compile(
        optimizer=keras.optimizers.sgd(lr=0.00000000005, momentum=0.9),
